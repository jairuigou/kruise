...
func (dsc *ReconcileDaemonSet) syncNodes(ctx context.Context, ds *appsv1alpha1.DaemonSet, podsToDelete, nodesNeedingDaemonPods []string, hash string) error {
	...
	klog.V(4).InfoS("Pods to delete for DaemonSet deleting", "daemonSet", klog.KObj(ds), "podsToDelete", podsToDelete, "count", deleteDiff)
		...
		if isSatisfied, unsatisfiedDuration := dsc.resourceVersionExpectations.IsSatisfied(pod); !isSatisfied {
			if unsatisfiedDuration >= kruiseExpectations.ExpectationTimeout {
				klog.ErrorS(fmt.Errorf("expectation unsatisfied resourceVersion overtime for DaemonSet, wait for pod updating"),
					"Expectation unsatisfied resourceVersion overtime for DaemonSet, wait for pod updating",
