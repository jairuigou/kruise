...
func (e *podEventHandler) Delete(ctx context.Context, evt event.DeleteEvent, q workqueue.RateLimitingInterface) {
	...
	if !ok {
		klog.ErrorS(fmt.Errorf("DeleteEvent parse pod failed"), "Skipped pod deletion event",
			"deleteStateUnknown", evt.DeleteStateUnknown, "obj", evt.Object)
		return
	}
...
func (e *pvcEventHandler) Delete(ctx context.Context, evt event.DeleteEvent, q workqueue.RateLimitingInterface) {
	...
	if !ok {
		klog.ErrorS(fmt.Errorf("DeleteEvent parse pvc failed"), "Skipped pvc deletion event",
			"deleteStateUnknown", evt.DeleteStateUnknown, "obj", evt.Object)
		return
	}
